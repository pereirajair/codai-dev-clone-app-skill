...
**IMPORTANT**: Polish requires **JavaScript execution** to read `getComputedStyle` residuals — this stage requires the **Chrome extension** (`mcp__claude-in-chrome__*`). BrowserMCP has no JS execution tool and cannot perform computed-style reads. Open the localhost clone with `mcp__claude-in-chrome__navigate` and do every residual check via `mcp__claude-in-chrome__javascript_tool`. One view at a time — polish agents run sequentially per page.
...
2. Open the clone with `mcp__claude-in-chrome__navigate` to `http://localhost:3000/{route}`, and take a visual screenshot (`mcp__claude-in-chrome__computer` screenshot) as an eyeball reference. Compare it against the recon baseline `01-recon/screenshots/{page}--desktop.png` at the same scroll position to confirm which element(s) sit under each residual selector.
...
Apply minimal, targeted property fixes to the page code for `{PAGE}` (no refactors, no new components, no shared-file edits). After each fix, re-run assert-styles and re-read the affected element's computed style via `mcp__claude-in-chrome__javascript_tool` (reload the route first with `mcp__claude-in-chrome__navigate`). If a fix does not reduce the residual delta — or makes the computed values diverge further from the spec — revert it.
...
Toggle dark mode and walk `{PAGE}` against the dark theme tokens in `css-variables.json` / `DESIGN.md` (read each element's computed background/text/border/shadow via `mcp__claude-in-chrome__javascript_tool` in the dark state):
